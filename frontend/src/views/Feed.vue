    ...
    <v-content>
      ...
      <v-row class="d-flex justify-center">
        <v-col
          ...
          md="3"
          lg="2"
          class="order-xs-3 order-sm-3 order-md-2 order-lg-1"
        >
          <section>
            <!-- Left Column -->
            <h1>Recent Media</h1>
            <template v-if="posts.length != 0">
              <div v-for="post in posts" :key="post._id">{{ post._id }}</div>
            </template>
            <template v-else>
        ...
        <v-col
          ...
          sm="12"
          lg="5"
          class="order-xs-1 order-sm-1 order-md-1 order-lg-2"
        >
          <!-- Middle column -->
          <v-btn @click="singlePostModal=true, newPost=true">new post</v-btn>
          <template v-if="posts">
            <v-card class="post" outlined tile ripple v-for="post in posts" :key="post._id">
              <SinglePost
                :authUser="authUser"
                :post="post"
                :token="token"
        ...
        <v-col
          ...
          md="12"
          lg="2"
          class="order-xs-2 order-sm-2 order-md-2 order-lg-3"
        >
          <section>
            <!-- Right Column -->
            <h1>Recommended</h1>
...
export default {
  ...
  created () {
    ...
    fetch(`http://206.189.215.72:3000/feed/posts/?page=${page}`, {
      headers: {
